...
import { Home, GraduationCap, Users, ClipboardCheck, BookOpen, UserCheck, FileCheck, CalendarDays, BarChart3, Megaphone, MessageSquare, LogOut, HelpCircle, Check, PanelLeftClose } from 'lucide-react';
...
const TABS = [
  { key: 'home', label: 'Home', icon: Home, href: '/faculty' },
  { key: 'courses', label: 'My Programmes', icon: GraduationCap, href: '/faculty/courses' },
  { key: 'students', label: 'Students', icon: Users, href: '/faculty/students' },
  { key: 'grading', label: 'Grading', icon: ClipboardCheck, href: '/faculty/grading' },
  { key: 'gradebook', label: 'Gradebook', icon: BookOpen, href: '/faculty/gradebook' },
  { key: 'attendance', label: 'Attendance', icon: UserCheck, href: '/faculty/attendance' },
  { key: 'exams', label: 'Exams', icon: FileCheck, href: '/faculty/exams' },
  { key: 'schedule', label: 'Schedule', icon: CalendarDays, href: '/faculty/schedule' },
  { key: 'reports', label: 'Reports', icon: BarChart3, href: '/faculty/reports' },
  { key: 'announcements', label: 'Announcements', icon: Megaphone, href: '/faculty/announcements' },
  { key: 'forums', label: 'Forums', icon: MessageSquare, href: '/faculty/forums' },
] as const;

const FACULTY = { name: 'Prof. Raghav Iyer', initials: 'RI', email: 'raghav.iyer@university.edu' };
  ...
  return (
    <aside style={{
      ...
      <nav style={{ padding: collapsed ? '12px 0 4px' : '14px 10px 4px', flex: 1, overflow: 'hidden' }}>
        {TABS.map(tab => {
          const Icon = tab.icon;
          const isActive = tab.href === '/faculty'
            ? pathname === '/faculty'
            : pathname?.startsWith(tab.href);
          return collapsed ? (
            <Link key={tab.key} href={tab.href}
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', height: 40, marginBottom: 2, borderRadius: 6, background: isActive ? 'rgba(255,255,255,0.10)' : 'transparent', textDecoration: 'none' }}
              title={tab.label}
            >
              <Icon size={18} strokeWidth={1.6} color={isActive ? TEXT_ACTIVE : ICON} />
            </Link>
          ) : (
            <Link key={tab.key} href={tab.href}
              style={{
      ...
      {!collapsed && (
        <div style={{ padding: '0 10px 12px', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px' }}>
            <div style={{ width: 32, height: 32, borderRadius: '50%', background: '#072FB5', display: 'grid', placeItems: 'center', fontSize: 11, fontWeight: 800, color: '#fff', flexShrink: 0 }}>{FACULTY.initials}</div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: TEXT_ACTIVE, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{FACULTY.name}</div>
              <div style={{ fontSize: 10, color: TEXT, marginTop: 1 }}>Faculty</div>
            </div>
