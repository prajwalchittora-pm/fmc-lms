        ...
        return (
          <div key={i} style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, alignItems:'center' }}>
            ...
            <select disabled={locked} value={sel??''} onChange={e=>{ const u=[...ans.value]; u[i]=e.target.value===''?null:parseInt(e.target.value); set({type:'matching',value:u}); }}
              style={{ padding:'11px 28px 11px 14px', fontSize:13, fontFamily:'var(--font-sans)', border:`1.5px solid ${sel!==null?'var(--blue-700)':'var(--border-subtle)'}`, borderRadius:'var(--radius-sm)', background:sel!==null?'var(--blue-50)':'#fff', color:'var(--text-primary)', outline:'none', cursor:locked?'default':'pointer', appearance:'none', WebkitAppearance:'none', backgroundImage:`url("data:image/svg+xml,%3Csvg width='10' height='6' viewBox='0 0 10 6' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 1L5 5L9 1' stroke='%23999' stroke-width='1.5' stroke-linecap='round' stroke-linejoin='round'/%3E%3C/svg%3E")`, backgroundRepeat:'no-repeat', backgroundPosition:'right 10px center' }}
            >
